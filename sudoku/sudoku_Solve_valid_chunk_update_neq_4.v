...
Axiom same_chunk_injective : forall (start:(map.Map.map Z Z)) (i:Z) (j:Z),
  (~ (same_chunk start i j)) -> ~ (i = j).

(* Why3 assumption *)
Definition wf_start_offsets (start:(map.Map.map Z Z)) (offsets:(map.Map.map Z
  Z)): Prop := forall (i:Z) (o:Z), (((0%Z <= o)%Z /\ (o < 9%Z)%Z) /\
  ((0%Z <= i)%Z /\ (i < 81%Z)%Z)) -> (is_index ((map.Map.get start
  i) + (map.Map.get offsets o))%Z).

Axiom update_not_same_chunk : forall (g:(map.Map.map Z Z)) (i:Z) (x:Z)
  (start:(map.Map.map Z Z)), forall (j:Z), ((is_index i) /\ ((is_index j) /\
  ~ (same_chunk start i j))) -> ((map.Map.get (map.Map.set g i x)
  j) = (map.Map.get g j)).

(* Why3 goal *)
Theorem valid_chunk_update_neq : forall (g:(map.Map.map Z Z)) (i:Z) (x:Z)
  (start:(map.Map.map Z Z)) (offsets:(map.Map.map Z Z)), forall (j:Z),
  (valid_chunk g j start offsets) -> ((wf_start_offsets start offsets) ->
  (((is_index i) /\ ((is_index j) /\ ~ (same_chunk start i j))) ->
  (valid_chunk (map.Map.set g i x) j start offsets))).
intros g i x start offsets j h1 h2 (h3,(h4,h5)).
