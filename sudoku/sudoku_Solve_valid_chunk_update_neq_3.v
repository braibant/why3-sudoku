...
Theorem valid_chunk_update_neq : forall (g:(map.Map.map Z Z)) (i:Z) (x:Z)
  ...
  (valid_chunk g j start offsets) -> (((is_index i) /\ ((is_index j) /\
  ~ (same_chunk start i j))) -> (valid_chunk (map.Map.set g i x) j start
  offsets)).
intros g i x start offsets j h1 (h2,(h3,h4)).
unfold valid_chunk. 
intros o1 o2. intros.
rewrite (update_not_same_chunk _ _ _ start). 
rewrite (update_not_same_chunk _ _ _ start). 
apply h1. auto. 
rewrite (update_not_same_chunk _ _ _ start) in H0. 
rewrite (update_not_same_chunk _ _ _ start) in H0.
auto.  
