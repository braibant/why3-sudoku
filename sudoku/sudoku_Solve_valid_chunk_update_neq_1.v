...
Theorem valid_chunk_update_neq : forall (g:(map.Map.map Z Z)) (i:Z) (x:Z)
  ...
  (valid_chunk g j start offsets) -> ((~ ((map.Map.get start
  i) = (map.Map.get start j))) -> ((~ (i = j)) -> ((is_grid g) ->
  (valid_chunk (map.Map.set g i x) j start offsets)))).
(* Why3 intros g i x start offsets j h1 h2 h3 h4. *)
intros g i x start offsets h1 j h2 h3.
